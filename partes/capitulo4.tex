...
\begin{description}[leftmargin=!,labelwidth=\widthof{\bfseries Homebrew}]
  ...
  \item [APT] Para instalar el compilador graciela en sistemas operativos
    derivados de Debian se hace a través de la herramienta APT, usando los
    comandos del fragmento de código~\ref{lst:apt}
...
\begin{lstlisting}[caption=Instrucciones para instalar el compilador de Graciela en Linux con APT, label={lst:apt}]
  # add-apt-repository ppa:graciela-usb/ppa
  # apt-get update
  # apt-get install graciela
\end{lstlisting}

\begin{lstlisting}[caption=Instrucciones para instalar el compilador de Graciela en macOS con Homebrew, label={lst:brew}]
