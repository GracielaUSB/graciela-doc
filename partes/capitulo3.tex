...
\texttt{where}, a la cual nos referiremos en adelante como \textit{la}
...
acoplamiento, escrita entre los símbolos \texttt{\{coupinv} y
\texttt{coupinv\}}, a la cual nos referiremos en adelante como el <<invariante
de acoplamiento>>.
...
implementación \texttt{Conjunto}, ambas haciendo uso de variables de tipo,
\texttt{T} y \texttt{N} respectivamente. Como se puede apreciar, el tipo
\texttt{Conjunto} implementa el TDA \texttt{ConjuntoA} con la variable de tipo
\texttt{N} sustituyendo a la variable de tipo \texttt{T}, por lo que cada
ocurrencia de \texttt{T} dentro de la especificación de \texttt{ConjuntoA}, será
sustituida por la variable de tipo \texttt{N}.
...
esta forma cualquier ocurrencia de \texttt{N} (incluyendo las ocurrencias de
\texttt{T} en \texttt{ConjuntoA} sustituidas en el paso anterior), dentro de la
especificación de \texttt{Conjunto}, es sustituida por el tipo \texttt{int}.
...
en cada uso. De esta forma, para un tipo \texttt{Diccionario(T0,T1)}, se guarda
la información sobre sus apariciones en declaraciones y los tipos básicos usados
en cada una. Si los usos fueron \texttt{Diccionario(int, char)} y
\texttt{Diccionario(int, float)}, al llegar a la fase de generación de código
...
esta forma, suponiendo que el procedimiento \texttt{agregar} del tipo
\texttt{Conjunto(X)} tiene un parámetro de tipo \texttt{Conjunto(X)}, y que en
el código principal del programa se declararon variables de los tipos
\texttt{Conjunto(int)} y \texttt{Conjunto(char)}, serán generadas en el código
...
de un procedimiento como \texttt{agregar (c : Conjunto(T), e : T)}, en el punto
de llamada \texttt{agregar(s, 3)}, donde \texttt{s} es de tipo
\texttt{Conjunto(int)}, primero se consigue la variable \texttt{s}. En este
momento, se puede asociar la variable de tipo \texttt{T} de la definición del
procedimiento \texttt{agregar} con el tipo \texttt{int}. Luego, cuando se
identifica el argumento \texttt{3}, se compara su tipo con el del parámetro
\texttt{e}, \texttt{T}, y como es una variable de tipo, se usa el tipo que fue
asociado a esta, \texttt{int}. Como coincide con el tipo de la expresión
\texttt{3}, esta llamada pasa la verificación de tipos y se verifica la
siguiente instrucción.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Tipos Algebráicos Libres %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Tipos Algebráicos Libres}

1. parte muy menor de la materia de algoritmos 2
2. dificil de integrar con el lenguaje actual
3. paradigma distinto, más funcional que imperativo
4. patter matching es *dificil*
5. mejores alternativas para su enseñanza *cough*Haskell*cough*
6. son material para un proyecto de grado en sí mismo
