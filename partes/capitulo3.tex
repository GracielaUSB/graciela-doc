...
\begin{figure}[h!]
  ...
  \centering
    \fboxsep=5mm
    \fboxrule=0.75pt
    \fcolorbox{gray}{white}{\includegraphics[width=0.5\textwidth]{diag2}}
    \label{fig:diag}
\end{figure}
...
%% PROGRAMAS DE PRUEBA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Colección de casos de prueba} \label{sect:tests}
...
acciones del Monad \texttt{IO}, como \inhas{compilar archivo} y 
\inhas{readFile "entrada"}, con combinadores propios de HUnit, como \inhas{@?=}, 
...
\inhas{simularConjunto}, escrito aprovechando la biblioteca de Haskell
\inhas{Data.Set}) para cada propiedad, es decir, una función en Haskell capaz de
generar la salida a partir de la entrada de manera correcta, así como una
instancia \inhas{Arbitrary} para las entradas de cada propiedad (en el ejemplo,
\inhas{EntradaConjunto}). El fragmento de código \ref{lst:quickex2} muestra una
sencilla instancia \inhas{Arbitrary} para el tipo \inhas{EntradaConjunto}. Puede
...
o imprimir el conjunto. Esta instancia \inhas{Arbitrary} genera listas de \inhas{200} instrucciones con
el combinador \inhas{vectorOf}, donde cada instrucción es generada por
\inhas{linea}. Este generador, a su vez, aprovecha el combinador \inhas{frequency}, el cual <<escoge>> un
generador entre la lista con una distribución aleatoria ponderada por los coeficientes. El generador \inhas{pure Imprimir} simplemente devuelve la instrucción \inhas{Imprimir},
mientras que los que hacen uso del operador \inhas{<$>} aprovechan la instancia
\inhas{Arbitrary} del tipo del elemento a usar en el conjunto para devolver una instrucción como \inhas{Insertar (Entero 3)}.

\begin{haskellcode}[caption=Ejemplo de instancia \inhas{Arbitrary}, label=lst:quickex2]
instance Arbitray EntradaConjunto where
  arbitrary = vectorOf 200 linea
    where
      linea = frequency
        [ (4, Insertar  <$> arbitrary) 
        , (3, Pertenece <$> arbitrary)
        , (1, pure Imprimir) ]
\end{haskellcode}

Con esta herramienta, se hizo innecesario escribir manualmente las entradas para
los programas de prueba y sus correspondientes salidas esperadas. En su lugar,
para cada programa de prueba se escribieron simuladores en Haskell de los
